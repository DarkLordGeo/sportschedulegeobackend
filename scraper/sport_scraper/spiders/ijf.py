...
class IjfSpider(scrapy.Spider):
    ...
    def parse(self, response: scrapy.http.Response) -> Iterable[EventItem | scrapy.Request]:
        ...
        candidate_events = response.css("[data-event], .event, .calendar-event, article")
    ...
    def parse_date_range(self, value: str) -> tuple[date | None, date | None]:
        value = self.clean_text(value)
        if not value:
            return None, None
        parts = re.split(r"\s+(?:to|-|–|—)\s+", value, maxsplit=1)
        start_date = self.parse_date_value(parts[0])
        end_date = self.parse_date_value(parts[1]) if len(parts) > 1 else None
